...
int main(int argc, char ** argv) {
	...
	numBlock = (M/NR_CHUNKS + threadsPerBlock - 1) / threadsPerBlock;
	...
	for (i = 0 ; i < NR_CHUNKS ; i++) {
		int chunk_rows = M/NR_CHUNKS;
		int offset_rows = i * chunk_rows;
		int stream_id = i % STREAM_NUMBER;
		//TODO: Copy a chunk of "A"  (M / NR_CHUNKS rows) over a stream to the GPU in "dev_A"
		cudaMemcpyAsync(&dev_A[offset_rows * N],
						&A[offset_rows * N],
		...
		cudaDGEMV_shmem<<<numBlock, threadsPerBlock, 0, stream[stream_id]>>>(
			&dev_A[offset_rows * N],
			dev_x,
			&dev_b[offset_rows],
			chunk_rows,
