...
__global__ void cudaDGEMV(float * A, float * x, float * b, int M, int N) {
	...
	// Each thread computes one element of the output vector b
	if (tid < M) {
		float sum = 0.0f;
		for (int j = 0; j < N; j++) {
			sum += A[tid * N + j] * x[j];
...
int main(int argc, char ** argv) {
	...
	//Assume a 1D block
	blockX = BLOCK_DIM;		// 1024 threads per block
	blockY = 1;
       	blockZ = 1;
	...
	//Assume a 1D grid
	gridX = (M + BLOCK_DIM - 1) / BLOCK_DIM; 	// Enough blocks to cover M elements
	gridY = 1;
	gridZ = 1;
	...
	err = cudaMemcpy(b, dev_b, M * sizeof(float), cudaMemcpyDeviceToHost);
	if (err != 0) {
		fprintf(stderr, "Error copying result back to CPU\n");
		exit(-1);
	}
	//----
