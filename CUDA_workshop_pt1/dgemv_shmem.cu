...
__global__ void cudaDGEMV_shmem(float * A, float * x, float * b, int M, int N) {
	...
	// Get the row index for this block (each block processes one row)
	int row = blockIdx.x;

	// Get the thread index within the block
	int tid = threadIdx.x;

	// Each thread computes partial sum for elements in this row
	float temp_sum = 0.0f;

	// Stride through the row, each thread handles multiple elements if N > BLOCK_DIM
	for (int col = tid; col < N; col += blockDim.x) {
		temp_sum += A[row * N + col] * x[col];
	}
	...
	// Linear reduction: thread 0 sums all values in shared memory
	if (tid == 0) {
		float total = 0.0f;
	...
	// Binary reduction: parallel reduction in log(N) steps
