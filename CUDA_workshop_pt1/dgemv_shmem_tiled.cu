...
__global__ void cudaDGEMV_shmem(float * A, float * x, float * b, int M, int N) {
	...
	__shared__ float x_tile[BLOCK_DIM];

	int row = blockIdx.x;
	int tid = threadIdx.x;

	float result = 0.0f;
	// Number of tiles needed to cover the entire vector x
	int num_tiles = (N + BLOCK_DIM - 1) / BLOCK_DIM;

	for (int tile = 0; tile < num_tiles; tile++) {
		int x_index = tile * BLOCK_DIM + tid;
		if (x_index < N) {
			x_tile[tid] = x[x_index];
		} else {
			x_tile[tid] = 0.0f;
		}
		...
		int col = tile * BLOCK_DIM + tid;

		if (col < N) {
			result += A[row * N + col] * x_tile[tid];
		}
		__syncthreads();
	}
	b[row] = result;
